...
All terms are nonnegative.  Summing (1.2), first over finite triangular regions and then using monotone convergence, gives

\[
\sum_{n\ge0}u_n
=1+\theta\sum_{n\ge0}u_n,
\]
...
The only imported BFG facts now used for (2.3) are the ratio-coupling construction, the suffix comparison leading to (2.1), and the explicit first-return formula (1.9).  The polynomial renewal estimate itself is internal.
