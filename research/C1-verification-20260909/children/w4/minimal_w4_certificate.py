...
def certify_center(center):
    ...
    return {
        ...
        "min_decimal_ldl_pivot_lower_endpoint": str(min(p[0] for p in pivots)),
        "min_decimal_ldl_pivot_lower_diagnostic": f"{min(p[0] for p in pivots):.18E}",
        "max_decimal_pivot_width_diagnostic": f"{max(p[1] - p[0] for p in pivots):.3E}",
