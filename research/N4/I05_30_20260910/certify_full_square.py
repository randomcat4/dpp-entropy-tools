...
rational interval boxes. No random search, optimizer, or import from adjacent
author scripts.
"""
...
def polynomial_interval(coeff,a,b,c,d):
    """Natural exact interval; variables are nonnegative."""
    lo=hi=F(0)
    ...
    return lo,hi
def f_lower(a,b):
    """Lower bound for f(c)=c^2 log(c^2) on rational [a,b]."""
    if b<=F(3,5):
