...
def run():
    ...
    def evaluate(x,meta):
        ...
        try:
            ...
            e3,Q3=np.linalg.eigh(-M[:2,:2]);e4,Q4=np.linalg.eigh(-M[2:,2:]);single_ok=min(e3[0],e4[0])>0
            if single_ok:
            ...
            records.append(rec);handle.write(json.dumps(rec)+'\n');handle.flush()
            ...
            core.COUNTS['rejected']+=1;failures.append({'index':index,'meta':meta,'x':x.tolist(),'error':repr(error)});return 1e3
