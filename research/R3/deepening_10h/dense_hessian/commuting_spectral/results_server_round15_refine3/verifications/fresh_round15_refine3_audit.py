#!/usr/bin/env python3
"""Fresh non-author audit for D10-H7 round15_refine3.

No author gate/search module is imported.  The script checks ledger accounting,
hashes, best JSON/NPZ consistency, and independently recomputes the strongest
stored point by exact-event Möbius inversion using Decimal arithmetic plus
Fraction LDL feasibility checks.
"""
...
BASE = HERE.parent
EXPECTED = {
    "source_npz": "32741ce56d53b0c3637172d1805b7810990616e6ff90ae0c94509787fdd521ef",
    "strongest_npz": "21bcb4d8867b2f7d32ee398432dfaa6f2d91976a4a959191d2da4d112153eb71",
...
def best_json_matches_row(best_json, best_row):
    mismatches = []
    for key, value in best_json.items():
        ...
        if isinstance(value, bool):
            ...
            ok = close_float(best_row[key], value)
        else:
...
def strongest_arrays():
    data = np.load(BASE / "results_0" / "best_case.npz")
...
def audit():
    started = time.time()
    ledger = ledger_audit()
    author = read_json(BASE / "recheck_best_refine3.json")
    best_json = read_json(BASE / "results_0" / "best_case.json")
    raw_K, raw_D, K, D = strongest_arrays()
    ...
    hashes = {
        ...
        "strongest_npz": sha256(BASE / "results_0" / "best_case.npz"),
        ...
        "recheck_best_refine3_py": sha256(BASE / "recheck_best_refine3.py"),
        "recheck_best_refine3_json": sha256(BASE / "recheck_best_refine3.json"),
    }
    checks = {
        ...
        "strongest_location_shard0_index4942": ledger["aggregate"]["best_shard"] == 0
        and ledger["aggregate"]["best_index"] == 4942,
        "strongest_label_three_basis": ledger["aggregate"]["best_label"] == "three_basis_rotations",
        ...
        "author_summary_best_location_matches": author["strongest_shard"] == 0
        and author["strongest_index"] == 4942,
        ...
        "scope_classification": {
            "ledger_and_manifest": "CORRECT" if all(
                ...
                for k in [
                    ...
                    "aggregate_sources_4",
                    "strongest_location_shard0_index4942",
                    "strongest_label_three_basis",
...
def main():
    report = audit()
    out = HERE / "fresh_round15_refine3_audit.json"
