...
def run(args: argparse.Namespace) -> None:
    ...
    source_metrics = base_metrics.copy()
    ...
    source_status = (
        "FLOAT_CANDIDATE"
        if float(source_metrics["rho_psd"]) > 1.0 + args.threshold
        or float(source_metrics["chord_gap"]) > args.gap_threshold
        else "NO_HIT"
    )
    source_row = {
        "index": -1,
    ...
    (out / "best_case.json").write_text(
        json.dumps(source_row, indent=2), encoding="utf-8"
    )
    if source_status == "FLOAT_CANDIDATE":
        candidate_dir = out / "candidate_cases"
        candidate_dir.mkdir(exist_ok=True)
        np.savez_compressed(
            candidate_dir / "candidate_source_base.npz",
            kernel=base_kernel,
            direction=base_direction,
            spectrum=base_spectrum,
            rates=base_rates,
        ...
        (candidate_dir / "candidate_source_base.json").write_text(
            json.dumps(source_row, indent=2), encoding="utf-8"
        )
    ...
    with (out / "candidate_ledger.csv").open("w", encoding="utf-8", newline="") as handle:
        ...
        for index in range(args.centers):
            ...
            if status == "FLOAT_CANDIDATE":
                candidate_dir = out / "candidate_cases"
                candidate_dir.mkdir(exist_ok=True)
                stem = f"candidate_{index:06d}"
                np.savez_compressed(
                    candidate_dir / f"{stem}.npz",
                    kernel=kernel,
                    direction=direction,
                    spectrum=spectrum,
                    rates=rates,
    ...
    manifest = {
        ...
        "source_rho": source_metrics["rho_psd"],
        "source_gap": source_metrics["chord_gap"],
        "best_rho_psd": best_metrics["rho_psd"],
