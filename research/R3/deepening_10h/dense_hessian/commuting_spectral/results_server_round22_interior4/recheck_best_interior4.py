"""Run the corrected high-precision gate on round-22 interior data."""
...
SPEC = importlib.util.spec_from_file_location("corrected_interior4_gate", SOURCE)
...
def sha256_with_source_alias(path: Path) -> str:
    """Preserve the copied source name while satisfying the inherited gate."""
    if path.name == "source.npz" and not path.exists():
        path = HERE / "interior_source_margin022.npz"
...
if __name__ == "__main__":
    ...
    inherited.replace(HERE / "recheck_best_interior4.json")
