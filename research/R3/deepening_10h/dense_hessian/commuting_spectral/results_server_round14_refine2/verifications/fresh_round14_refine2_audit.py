#!/usr/bin/env python3
"""Fresh non-author audit for round14_refine2.

This script intentionally does not import the author gate or search code.  It
parses the copied ledgers/manifests, computes hashes, and independently
recomputes the strongest stored NPZ with Decimal exact-event Möbius inversion
and Fraction LDL feasibility checks.
"""
...
N = 12
EXPECTED = {
    "source_npz": "4bda0a82ca642b5e804bca498107a89a1c81a3a7eb79ce6aca2712e4d594cb72",
    "strongest_npz": "32741ce56d53b0c3637172d1805b7810990616e6ff90ae0c94509787fdd521ef",
...
def numeric_close(a, b, tol=1e-14) -> bool:
...
def ledger_audit():
    ...
    for shard in range(4):
        ...
        best_matches_json = True
        mismatches = []
        for key, value in best_json.items():
            if key not in best_row:
                continue
            if isinstance(value, bool):
                ok = parse_bool(best_row[key]) == value
            elif isinstance(value, int):
                ok = int(best_row[key]) == value
            elif isinstance(value, float):
                ok = numeric_close(best_row[key], value)
            else:
                ok = str(best_row[key]) == str(value)
            if not ok:
                best_matches_json = False
                mismatches.append({"field": key, "ledger": best_row[key], "json": value})

        positive_gap_rows = [row for row in rows if float(row["chord_gap"]) > 0.0]
        non_no_hit = [row for row in rows if row["status"] != "NO_HIT"]
        accepted_proposals = [row for row in proposal_rows if parse_bool(row["accepted"])]
        ...
        per_shard.append(
            {
                ...
                "manifest_ledger_rows_including_source": manifest.get("ledger_rows_including_source"),
                ...
                "accepted_count_matches_manifest": len(accepted_proposals) == int(manifest["accepted_count"]),
                "non_no_hit_rows": len(non_no_hit),
                "positive_gap_rows": len(positive_gap_rows),
                "maximum_gap_all_rows": max(float(row["chord_gap"]) for row in rows),
                ...
                "best_matches_manifest_rho": numeric_close(best_row["rho_psd"], manifest["best_rho_psd"]),
                "best_matches_json": best_matches_json,
                "best_json_mismatches": mismatches,
                "best_case_json_sha256": sha256(shard_dir / "best_case.json"),
                "best_case_npz_sha256": sha256(shard_dir / "best_case.npz"),
                "run_log_lines": len(run_log.splitlines()),
                "run_log_mentions_seed": str(manifest.get("seed")) in run_log,
            }
        )
    best_global = max(all_rows, key=lambda row: float(row["rho_psd"]))
    return {
        "per_shard": per_shard,
        "aggregate": {
            ...
            "best_shard": int(best_global["_shard"]),
            "best_index": int(best_global["index"]),
            "best_rho_psd": float(best_global["rho_psd"]),
            "best_gap": float(best_global["chord_gap"]),
        },
    }


def decimal_matrix_from_numpy(matrix: np.ndarray) -> list[list[Decimal]]:
    return [[Decimal(repr(float(value))) for value in row] for row in matrix]


def decimal_det_inverse(matrix_input: list[list[Decimal]]):
    n = len(matrix_input)
...
def decimal_det(matrix_input: list[list[Decimal]]) -> Decimal:
    n = len(matrix_input)
    if n == 0:
        return Decimal(1)
    matrix = [row[:] for row in matrix_input]
...
def superset_mobius(values: list[Decimal], n: int) -> list[Decimal]:
    out = values[:]
...
def inclusion_derivatives(K, D):
    size = 1 << len(K)
    inc = [Decimal(0) for _ in range(size)]
    inc1 = [Decimal(0) for _ in range(size)]
    inc2 = [Decimal(0) for _ in range(size)]
    inc[0] = Decimal(1)
    for mask in range(1, size):
        idx = [i for i in range(len(K)) if (mask >> i) & 1]
        K_sub = principal(K, idx)
        D_sub = principal(D, idx)
        det, inv = decimal_det_inverse(K_sub)
        B = matmul(inv, D_sub)
...
def atoms_only(K):
    ...
    for mask in range(1, size):
        idx = [i for i in range(len(K)) if (mask >> i) & 1]
        inc[mask] = decimal_det(principal(K, idx))
    return superset_mobius(inc, len(K))


def entropy_from_decimal_matrix(K):
    p = atoms_only(K)
    if min(p) <= 0:
        raise ArithmeticError("nonpositive chord atom")
    total = sum(p)
    entropy = -sum(prob * prob.ln() for prob in p)
    return +entropy, +total, +min(p)


def directional_decimal_mobius(K_np: np.ndarray, D_np: np.ndarray, precision: int):
    with localcontext() as context:
        context.prec = precision
        K = decimal_matrix_from_numpy(K_np)
        D = decimal_matrix_from_numpy(D_np)
...
def entropy_decimal_mobius(K_np: np.ndarray, precision: int):
    with localcontext() as context:
        context.prec = precision
        K = decimal_matrix_from_numpy(K_np)
        return entropy_from_decimal_matrix(K)


def chord_decimal_mobius(K_np: np.ndarray, D_np: np.ndarray, steps: list[str], precision: int):
    with localcontext() as context:
        context.prec = precision
        # Center and endpoints are evaluated on the same Decimal line:
        # K(t) = K + Decimal(step) * D, after the same repr(float)
        # symmetrization/rationalization used by the derivative and Fraction
        # checks.  This avoids inserting an extra float multiplication in the
        # three-chord gate.
        K = decimal_matrix_from_numpy(K_np)
        D = decimal_matrix_from_numpy(D_np)
        center, center_sum, center_min = entropy_from_decimal_matrix(K)
        rows = []
        for step_string in steps:
            ...
            h_minus, sum_minus, min_minus = entropy_from_decimal_matrix(K_minus)
            h_plus, sum_plus, min_plus = entropy_from_decimal_matrix(K_plus)
...
def fraction_matrix_from_numpy(matrix: np.ndarray):
    return [[Fraction(repr(float(value))) for value in row] for row in matrix]


def ldl_pivots_fraction(matrix):
    n = len(matrix)
...
def ldl_report(matrix):
    pivots = ldl_pivots_fraction(matrix)
    positive = [p > 0 for p in pivots]
    minimum = min(pivots)
    return {
        "count": len(pivots),
...
def fraction_feasibility(K_np: np.ndarray, D_np: np.ndarray):
    ...
    for step in (-radius, radius):
        Kt = add_fraction(K, D, step)
        K_margin = add_fraction(Kt, eye, -margin)
        I_minus_K_margin = add_fraction(add_fraction(eye, Kt, Fraction(-1)), eye, -margin)
        endpoints.append(
            {
                "step": str(step),
...
def strongest_npz_arrays():
    path = BASE / "results_2" / "best_case.npz"
    data = np.load(path)
...
def audit():
    started = time.time()
    ledger = ledger_audit()
    author_summary = read_json(BASE / "recheck_best_refine2.json")
    raw_K, raw_D, K, D = strongest_npz_arrays()
    best_row = ledger["aggregate"]
    chord_steps = [
        str(read_json(BASE / "results_2" / "best_case.json")["chord_step"]),
        "0.001",
        "0.0001",
    ]
    decimal_90 = directional_decimal_mobius(K, D, 90)
    chords_90 = chord_decimal_mobius(K, D, chord_steps, 90)
    feasibility = fraction_feasibility(K, D)
    hashes = {
        "source_npz": sha256(BASE / "source.npz"),
        "strongest_npz": sha256(BASE / "results_2" / "best_case.npz"),
        ...
        "recheck_best_refine2_py": sha256(BASE / "recheck_best_refine2.py"),
        "recheck_best_refine2_json": sha256(BASE / "recheck_best_refine2.json"),
    }
    checks = {
        ...
        "manifest_status_complete": all(
            row["manifest_status"] == "SCOUT_COMPLETE" for row in ledger["per_shard"]
        ),
        "manifest_exit_codes_zero": all(row["manifest_exit_code"] == 0 for row in ledger["per_shard"]),
        "manifest_positive_counts_zero": all(
            int(row["manifest_positive_count"]) == 0 for row in ledger["per_shard"]
        ),
        "accepted_counts_match_manifest": all(
            row["accepted_count_matches_manifest"] for row in ledger["per_shard"]
        ),
        ...
        "strongest_location_shard2_index4998": best_row["best_shard"] == 2
        and best_row["best_index"] == 4998,
        "strongest_json_matches_ledger": all(row["best_matches_json"] for row in ledger["per_shard"]),
        "author_summary_status_matches": author_summary["status"] == "HIGH_PRECISION_STABLE_NEGATIVE",
        "author_summary_best_location_matches": author_summary["strongest_shard"] == 2
        and author_summary["strongest_index"] == 4998,
        ...
        "fraction_endpoints_strict": all(
            item["K_minus_margin_I"]["all_positive"]
            and item["I_minus_K_minus_margin_I"]["all_positive"]
    ...
    return {
        ...
        "strongest": {
            "shard": 2,
            "index": 4998,
            "npz_sha256": hashes["strongest_npz"],
        ...
        "scope_classification": {
            "ledger_and_manifest": "CORRECT" if all(
                ...
                for k in [
                    ...
                    "aggregate_sources_4",
                    "strongest_location_shard2_index4998",
...
def main():
    report = audit()
    out = HERE / "fresh_round14_refine2_audit.json"
