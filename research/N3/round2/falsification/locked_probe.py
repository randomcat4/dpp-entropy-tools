...
def cofactor_matrix(K,p):
    ...
    def value(D):
        def cf(i,j):
            ...
            return (-1)**(i+j)*(D[r[0],c[0]]*D[r[1],c[1]]-D[r[0],c[1]]*D[r[1],c[0]])
        adj=mp.matrix([[cf(j,i) for j in range(3)] for i in range(3)])
...
def main():
    start=time.time();mp.mp.dps=100;results=[];total=0;print('START_PID',os.getpid(),flush=True)
