...
| 106 | `a175bf1d1b5a8c16537aa4924731de426dfc882d` | **MIXED: SUPERSEDED + EVIDENCE_INSUFFICIENT** | The load-bearing `A_p`, `p>=1`, local-concavity theorem is strictly contained in accepted PR117 and is superseded.  The separate centered Fisher expansion asserted for `p>=1/2` was not needed by the successor integration and has no independent version-bound review; it remains non-importable as a standalone claim. |
...
- PR106 standalone `p>=1/2` centered Fisher theorem, if anyone wants to import it separately.
