...
See `frozen_contract.md` for acceptance and stopping rules, `STATUS.md` for outcomes, and `provenance.md` for role separation. The original run and its preserved failure are in `compute/REPORT.md`; the remaining-window repair, determinant outputs and coefficient candidate are in `resume/`. The reviewed structural reduction and positive seed are in `structure/`, with their nonauthor reviews in `structure_review/`.
