...
def gershgorin_rows_json(P, margins, write_exact: bool):
    rows = []
    for i in range(NVAR):
        ...
        row = {
            ...
            "strict_positive_margin_checked_by_fraction": margins[i] > 0,
        }
        if write_exact:
            row.update(
                {
...
def run(
    ...
    write_exact_gershgorin: bool,
    only_center: str | None,
):
    started = time.time()
    ...
    if only_center is not None:
        centers = [c for c in centers if c["name"] == only_center]
        if not centers:
            raise ValueError(f"requested --only-center {only_center!r} was not found")
    ...
    metadata = {
        ...
        "write_exact_gershgorin": write_exact_gershgorin,
        "only_center": only_center,
        "original_center_count": original_center_count,
                ...
                try:
                    ...
                    if ok:
                        center_accepted = {
                            ...
                            "preconditioned_gershgorin_rows": gershgorin_rows_json(P, margins, write_exact_gershgorin),
                            "gershgorin_margins_decimal": [dec(m) for m in margins],
                        ...
                        checkpoint = out_dir / f"accepted_box_attempt_{attempts:03d}_{center['name']}.json"
                        write_json_atomic(checkpoint, center_accepted)
                        with (out_dir / "accepted_boxes_index.jsonl").open("a", encoding="utf-8") as index_file:
                            index_file.write(json.dumps({"attempt": attempts, "center": center["name"], "path": str(checkpoint)}) + "\n")
...
def main(argv=None):
    ...
    parser.add_argument("--write-exact-gershgorin", action="store_true")
    parser.add_argument("--only-center", type=str, default=None)
